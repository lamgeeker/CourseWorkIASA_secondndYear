...
namespace airCompany
{
    class AirCompany
    {
        ...
        public void CountCapAndLoad(Plane[] planes)
        {
            if (planes.Length == 0)
            {
                MessageBox.Show("Список літаків порожній. Додайте літаки перед підрахунком.", "Помилка");
                return;
            }
            ...
            MessageBox.Show(
                $"Загальна місткість усіх літаків: {AllCap} пасажирів\n" +
                $"Загальна вантажопідйомність: {AllLoad} кг",
